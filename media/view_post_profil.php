...
if(isset($_POST['proses']))
{
	...
	if($_POST["proses"] == "fetch_comment")
	{
		...
		if($statement->execute())
		{
			...
			foreach($result as $row)
			{
				...
				$output .= '
				'.$profile_image.'	
	              <div class="comment-text" style="margin-top: 0px; margin-bottom: 0px;">
	                <span class="username">
	                  '.$row["username"].'
  ...
  if($_POST['proses'] == 'pengikut_profil_cari')
	{
        $query = "
        SELECT * FROM user 
        INNER JOIN follow 
        ...
        foreach($result as $row)
        {
          ...
          <div class="row">
            <div class="col-xs-9">
              <div class="user-block">
                '.$profile_image.'
                <span class="username"><a href="profil_cari.php?data='.$row["user_id"].'">'.$row["nama_depan"].'</a></span>
                      <span class="description">'.$row["follower_number"].' Pengikut</span>
                  </div>
              </div>
              <div class="col-xs-3 text-center">
                '.make_follow_button($connect, $_POST["user_id"], $_SESSION["user_id"]).'
              </div>
            </div>
            </div>
          ';
	...
	if($_POST['proses'] == 'mengikuti_profil_cari')
	{
        $query = "
        SELECT * FROM user 
        INNER JOIN follow 
